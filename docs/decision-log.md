...
| 2026-06-07 | Keep the RAG assistant as a deterministic mock placeholder until retrieval, citations, and deployment architecture are implemented. | Superseded | Prior assistant implementation before 2026-06-08 static retrieval update | Avoided implying that real retrieval or vector search was available before the static retrieval implementation. |
| 2026-06-08 | Implement the assistant as deterministic static retrieval with cited site sources, while leaving vector search and model-backed generation as future work. | Active | `src/lib/rag.ts`; `src/components/chat/ChatPanel.tsx`; `src/app/api/chat/route.ts`; `docs/rag-assistant-architecture.md` | Makes the assistant useful on GitHub Pages without requiring a runtime backend or overstating vector/model RAG maturity. |
...
- Define whether future vector/model-backed RAG should use a separate backend, a generated local index, or a hybrid deployment model.
